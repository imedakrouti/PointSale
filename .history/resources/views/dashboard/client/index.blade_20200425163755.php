          ...
          <div class="col-12">
            <div class="card card-primary card-outline">
              ...
              <h3 class="card-title mb-2"><i class="fa fa-client"></i> @lang('site.clients')
                @if($clients)
                <span class="badge badge-info mx-2" >{{$clients ? 1 : 0}}</span>
                @endif
                    ...
                    <thead class="bg-primary">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ 1}}</td>
                                    <td>{{ $clients->name }}</td>
                                    <td>{{ is_array($clients->phone) ? implode('-',$clients->phone):$clients->phone}}</td>
                                    <td>{{ $clients->adress}}</td>
                                    <td>
                                        @if(auth()->user()->hasPermission('create_orders'))
                                        <a href="{{ route('dashboard.client.order.create',$clients->id) }}" class="btn btn-primary">@lang('site.add_order')</a>
                                        @else
                                    ...
                                    @if (auth()->user()->haspermission('update_clients'))
                                       <a href="{{ route('dashboard.client.edit', $clients->id) }}" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i> @lang('site.edit')</a>
                                    @else
                                    <button class='btn btn-warning disabled' >@lang('site.update')</button>
                                    @endif
                                           @if(auth::user()->hasPermission('delete_clients'))
                                             <form action="{{ route('dashboard.client.destroy', $clients->id) }}" method="post" style="display: inline-block">
                                                {{ csrf_field() }}
                                                {{ method_field('delete') }}
